...
export default function OrderOTP({ order }: { order: any }) {
    ...
    const otpDigits = String(order.deliveryOtp).slice(0, 6).split("");
        ...
        <div className="bg-gradient-to-r from-app-green to-app-green-light rounded-2xl p-6 text-white">
            ...
            <div className="flex gap-2 mt-2">
                {otpDigits.map((digit: string, i: number) => (
                    <div key={i} className="w-11 h-[3.25rem] rounded-xl bg-white/15 flex-center text-2xl font-mono font-bold tracking-wider">
                        {digit}
                    </div>
                ))}
            </div>
