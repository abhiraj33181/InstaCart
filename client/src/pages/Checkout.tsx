...
const Checkout = () => {
  ...
  const { currency, items, cartTotal, clearCart } = useCart();
  const { user } = useAuth();
  ...
  // populate address from user defaul address
  useEffect(() => {
        ...
        <div className="flex items-center gap-2 mb-8">
          {steps.map((s, i) => (
            <div key={s.key} className="flex items-center gap-2">
              <button className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${step === s.key ? "bg-app-green text-white" : "bg-white text-app-text-light"}`} onClick={() => setStep(s.key)}>
          ...
          <div className="bg-white rounded-2xl p-5 h-fit sticky top-24">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {deliveryFee === 0 ? <span className="text-app-success">Free</span> : <span> : `${currency}${deliveryFee.toFixed(2)}`</span>}
                </span>
              </div>
