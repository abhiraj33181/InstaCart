  ...
  return (
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* header */}
        <button className="flex items-center gap-2 text-sm text-app-text-light hover:text-app-green mb-6 transition-colors" onClick={() => navigate('/orders')}>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <span className={`px-4 py-1.5 text-sm font-semibold rounded-full ${order?.status ===
            'Delivered' ? 'bg-green-100 text-green-700' : order?.status === 'Cancelled' ? 'bg-red-100 text-red-700' : 'bg-app-orange/10 text-app-orange'
            }`}>
            {order!.status}
          </span>
        </div>
