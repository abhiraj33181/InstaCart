    ...
    <div className="min-h-screen bg-app-cream mb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {tabs.map(tab => (
            <button key={tab} className={`px-4 py-2 text-sm font-medium rounded-xl whitespace-nowrap transition-colors ${activeTab === tab ? 'bg-app-green text-white' : "bg-white text-app-text-light hover:bg-app-cream"}`} onClick={() => setActiveTab(tab)}>
                ...
                <Link key={order.id} to={`/orders/${order.id}`} className="block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {order.items.slice(0,4).map((item, index) => (
                      <img src={item.image} alt={item.name} key={index} className="size-12 sm:size-16 rounded-lg object-cover border border-app-border" />
                    ))}
