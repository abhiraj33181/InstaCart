...
const ProductPage = () => {
  const { id } = useParams()
  const { currency, items, addToCart, updateQuantity, removeFromCart } = useCart()
  ...
  useEffect(() => {
    ...
    }).then(({data}) => setRelatedProducts(data.products.filter((p : Product) => p.id !== id))).catch(() => navigate('/products')).finally(() => setLoading(false))
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* back button  */}
        <button className="mb-6 flex items-center gap-1.5 text-sm text-app-text-light hover:text-app-green transition-colors" onClick={() => navigate(-1)}>
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-app-border rounded-xl overflow-hidden">
                  ...
                  <span className="px-5 text-sm font-medium min-w-10 text-center">{displayQuantity}</span>
